...
// Movie actions
import { getPopularTvShowsAXN } from './popularActions';
...
describe('`getPopularTvShowsAXN`', () => {
  ...
  it('Should update `popularRXS.tvShows` correctly on successful API request', () => {
    ...
    return store.dispatch(getPopularTvShowsAXN()).then(() => {
      const newState = store.getState();
      expect(newState.popularRXS.tvShows).toStrictEqual(expectedRes.results);
    });
  });
