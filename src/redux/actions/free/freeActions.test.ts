...
// Movie actions
import { getFreeTvShowsAXN } from './freeActions';
...
describe('`getFreeTvShowsAXN`', () => {
  ...
  it('Should update `freeRXS.tvShows` correctly on successful API request', () => {
    ...
    return store.dispatch(getFreeTvShowsAXN()).then(() => {
      const newState = store.getState();
      expect(newState.freeRXS.tvShows).toStrictEqual(expectedRes.results);
    });
  });
